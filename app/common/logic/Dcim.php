...
class Dcim
{
                    ...
                    if (!empty($res["data"])) {
                        ...
                        if (isset($res["last_result"]) && $product["show_last_act_message"] == 1 && ($res["last_result"]["type"] == 1 || $res["last_result"]["type"] == 2)) {
                            if ($res["last_result"]["task_type"] == 3) {
                                ...
                                if ($res["last_result"]["task_type"] == 2) {
                                    $info = "重置密码";
                                } else {
                                    if ($res["last_result"]["task_type"] == 1) {
                                        $info = "救援系统";
                                    } else {
                                        $info = "重装系统";
                                    }
                ...
                if ($res["status"] == 200) {
                    ...
                    if ($res["power"] == "on") {
                        ...
                        if ($res["power"] == "off") {
                            ...
                            if ($res["power"] == "suspend") {
                                $result["data"]["status"] = "suspend";
                                $result["data"]["des"] = "暂停";
                            } else {
                                if ($res["power"] == "wait_reboot") {
    ...
    public function assignServer($id, $dcimid)
    {
        ...
        $product = \think\Db::name("host")->alias("a")->field("a.serverid,a.regdate,a.dcimid,b.config_option1,a.uid,a.nextduedate,b.host")->leftJoin("products b", "a.productid=b.id")->where("b.type", "dcim")->where("a.id", $id)->find();
        if (empty($product)) {
            $result["status"] = 400;
            $result["msg"] = lang("ID_ERROR");
            return $result;
        ...
        $is_assign = \think\Db::name("host")->where("serverid", $product["serverid"])->where("dcimid", $dcimid)->find();
        if (!empty($is_assign)) {
            $result["status"] = 400;
